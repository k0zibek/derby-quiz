...
export default function JoinPage() {
  ...
  useEffect(() => {
    ...
    if (savedPlayerId) {
      socket.emit("player:rejoin", { code, playerId: savedPlayerId }, (res) => {
        if (res?.ok) {
          setJoined(true);
  ...
  const handleJoin = () => {
    ...
    socket.emit("player:join", { code, name }, (res) => {
      ...
      if (!res?.ok) {
        alert(res?.error || "Не удалось подключиться");
        return;
      }
  ...
  const submitAnswer = (optionIndex) => {
    ...
    socket.emit("player:submitAnswer", { code, playerId, optionIndex }, (res) => {
      if (!res?.ok) {
        alert(res?.error || "Не удалось отправить ответ");
        return;
      }
        ...
        <section className="card mt-24">
          ...
          {session?.status === "result" && (
            <div className="question-box">
              ...
              <div
                className={
                  answerFeedback.includes("Правильно")
                    ? "badge badge-success"
                    : "badge badge-danger"
                }
